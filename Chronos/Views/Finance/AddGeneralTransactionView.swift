...
struct AddGeneralTransactionView: View {
    ...
    var mainCategories: [String] {
        transactionType == .expense
            ? lifestyleManager.mainCategories
            : lifestyleManager.incomeCategories
    }
    ...
    // MARK: - 拆分子 View 解決 type-check 超時問題
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(mainCategories, id: \.self) { cat in
                        CategoryChip(
                            title: cat,
                            isSelected: selectedMainCategory == cat,
    ...
    private func initCategories() {
        guard selectedMainCategory.isEmpty else { return }
        selectedMainCategory = mainCategories.first ?? ""
        selectedSubCategory = lifestyleManager.getSubCategories(for: selectedMainCategory).first ?? ""
    }
